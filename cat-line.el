...
(defcustom cat-line-position 'top
...
(defvar cat-line-collection-empty
  `(((cat-line-tall-space)))
...
(defvar cat-line-format-spec-default
  '(:left
    ...
    ((nil (cat-line-segment-misc-info))
     (" " (cat-line-git))
     (" " (cat-line-segment-persp-name))
     (" " (cat-line-segment-major-mode))
     ;; TODO: flycheck/flymake checker
     (" " (cat-line-segment-process))
     ;; (" " "")
     )))
...
(defun cat-line--generate-format ()
  '(:eval (cat-line-format-spec--process cat-line-format-spec)))
